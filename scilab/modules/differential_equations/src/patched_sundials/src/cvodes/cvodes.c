...
/* SUNDIALS EXTENSION */
static int cvHinFixed(CVodeMem cv_mem, sunrealtype tout, sunrealtype *tret);
...
{
  long int n_points;
  sunrealtype interval_size, hmax, test_div, floor_test;
...
static int cvStepDoPri(CVodeMem cv_mem)
{
    ...
    sunrealtype c2, c3, c4, c5, b1, b3, b4, b5, b6, d1, d3, d4, d5, d6, d7;
    sunrealtype a21, a31, a32, a41, a42, a43, a51, a52, a53, a54, a61, a62, a63, a64, a65, a71, a73, a74, a75, a76;
    a21 =  1. / 5.;
    a31 =  3. / 40.;
    a32 =  9. / 40.;
    a41 =  44. / 45.;
    ...
    if (retval < 0)
    {
        cvProcessError(cv_mem, CV_RHSFUNC_FAIL, __LINE__,"Dormand-Prince", "cvStepDoPri", MSGCV_RHSFUNC_FAILED, cv_mem->cv_tn);
        return(CV_RHSFUNC_FAIL);
    }
    if (retval > 0)
    {
        cvProcessError(cv_mem, CV_FIRST_RHSFUNC_ERR, __LINE__,"Dormand-Prince", "cvStepDoPri", MSGCV_RHSFUNC_FIRST);
        return(CV_FIRST_RHSFUNC_ERR);
    }
...
static int cvStepExpRK(CVodeMem cv_mem)
{
    ...
    if (retval < 0)
    {
        cvProcessError(cv_mem, CV_RHSFUNC_FAIL, __LINE__, "Runge-Kutta", "cvStepExpRK", MSGCV_RHSFUNC_FAILED, cv_mem->cv_tn);
        return(CV_RHSFUNC_FAIL);
    }
    if (retval > 0)
    {
        cvProcessError(cv_mem, CV_FIRST_RHSFUNC_ERR, __LINE__, "Runge-Kutta", "cvStepExpRK", MSGCV_RHSFUNC_FIRST);
        return(CV_FIRST_RHSFUNC_ERR);
    }
...
static int cvStepImpRK(CVodeMem cv_mem)
{
    ...
    sunrealtype difference;

    /* Coefficients */
    sunrealtype a11, a21, a22, a31, a32, a33, b1, b2, b3, c1, c2, c3;
    ...
    while (nb_iter <= maxcor)    /* Same operations as above, but with K[i] updated and store result in y to compare with cv_mem->cv_tempv */
    {
        ...
        if (retval < 0)
        {
            cvProcessError(cv_mem, CV_RHSFUNC_FAIL, __LINE__, "Runge-Kutta", "cvStepImpRK", MSGCV_RHSFUNC_FAILED, cv_mem->cv_tn);
            return(CV_RHSFUNC_FAIL);
        }
        if (retval > 0)
        {
            cvProcessError(cv_mem, CV_FIRST_RHSFUNC_ERR, __LINE__, "Runge-Kutta", "cvStepImpRK", MSGCV_RHSFUNC_FIRST);
            return(CV_FIRST_RHSFUNC_ERR);
        }
...
static int cvStepCRANI(CVodeMem cv_mem)
{
    ...
    while (nb_iter <= maxcor)    /* Same operations as above, but with K[i] updated and store result in y to compare with cv_mem->cv_tempv */
    {
        ...
        if (retval < 0)
        {
            cvProcessError(cv_mem, CV_RHSFUNC_FAIL, __LINE__, "Runge-Kutta", "cvStepCRANI", MSGCV_RHSFUNC_FAILED, cv_mem->cv_tn);
            return(CV_RHSFUNC_FAIL);
        }
        if (retval > 0)
        {
            cvProcessError(cv_mem, CV_FIRST_RHSFUNC_ERR, __LINE__, "Runge-Kutta", "cvStepCRANI", MSGCV_RHSFUNC_FIRST);
            return(CV_FIRST_RHSFUNC_ERR);
        }
...
static int cvRootfindExt(CVodeMem cv_mem)
{
    ...
    sunbooleantype zroot, umroot, sgnchg;
