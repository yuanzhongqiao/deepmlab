        ...
        if (SVG.equals(localName)) {
            ...
            if (isLocalized != null) {
                ...
                if ("ru_RU".equals(language) && HTMLDocbookTagConverter.containsCyrillic(buffer)) {
                    getConverter().error(new SAXParseException("SVG code contains cyrillic character. The tag <svg> should contain the attribute scilab:localized=\"true\" at line " + line + ".", null));
                } else if ("ja_JP".equals(language) && HTMLDocbookTagConverter.containsCJK(buffer)) {
                    getConverter().error(new SAXParseException("SVG code contains CJK character. The tag <svg> should contain the attribute scilab:localized=\"true\" at line " + line + ".", null));
                }
                f = new File(outputDir, BASENAME + getConverter().getCurrentBaseName() + "_" + (compt++) + ".png");
            }
